...
) implements CraftingRecipe
{
    ...
    public ItemStack assemble(CraftingInput input, HolderLookup.Provider registries)
    {
        ...
        for (int i = 0; i < input.size(); i++)
        {
            final Cordage carried = input.getItem(i).get(TerraVeraDataComponents.CORDAGE.get());
            if (carried != null && carried.strength() > strength)
            {
        ...
        out.set(TerraVeraDataComponents.CORDAGE.get(), new Cordage(Math.min(1f, strength), source));
        return out;
    }
